...
export function bookEndList(numbers: number[]): number[] {
    ...
    let numberLength: number = numbers.length;
    if (numberLength === 0) {
        return newNumbers;
    }
    if (numberLength === 1) {
    ...
    newNumbers = [numbers[0], numbers[numberLength - 1]];
    return newNumbers;
}
...
export function tripleNumbers(numbers: number[]): number[] {
    let tripledNums: number[] = [];
    tripledNums = numbers.map((num: number): number => num * 3);
    return tripledNums;
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    ...
    newStrings = messages.map((message: string): string =>
        message[message.length - 1] === "!" ? message.toUpperCase() : message,
    );
    newStrings = newStrings.filter(
...
export function makeMath(addends: number[]): string {
    ...
    let addStr: string = addends.reduce(
        (currStr: string, num: number) =>
            currStr[currStr.length - 1] === "=" ?
                currStr + num.toString()
            :   currStr + "+" + num.toString(),
        sum.toString() + "=",
    );
    return addStr;
